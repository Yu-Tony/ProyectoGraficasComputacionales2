...
public:
	BillboardRR(const wchar_t* billb, float xx, float zz, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, float anchura, float altura)
	{
		...
		CargaParametros(billb, anchura, altura);
	}
	BillboardRR(const wchar_t* billb, float xx, float zz, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, float anchura, float altura, int n)
	{
		//copiamos el device y el device context a la clase terreno
		d3dContext = D3DContext;
		d3dDevice = D3DDevice;
		...
		frontal = D3DXVECTOR3(0, 0, 1);
		//aqui cargamos las texturas de alturas y el cesped
		CargaParametros(billb, anchura, altura,n);
	}
	~BillboardRR()
	{
		//libera recursos
	...
	bool CargaParametros(const wchar_t* billb, float anchura, float altura)
	{
		...
		vertices[0].pos.x = 0;
		vertices[0].pos.y = 0;
		vertices[0].pos.z = -1 * anchura;
		vertices[0].UV.x = 0;
		vertices[0].UV.y = 1;

		vertices[1].pos.x = 0;
		vertices[1].pos.y = 2 * altura;
		vertices[1].pos.z = -1 * anchura;
		vertices[1].UV.x = 0;
		vertices[1].UV.y = 0;

		vertices[2].pos.x = 0;
		vertices[2].pos.y = 2 * altura;
		vertices[2].pos.z = 1 * anchura;
		vertices[2].UV.x = 1;
		vertices[2].UV.y = 0;

		vertices[3].pos.x = 0;
		vertices[3].pos.y = 0;
		vertices[3].pos.z = 1 * anchura;
		vertices[3].UV.x = 1;
		vertices[3].UV.y = 1;
	...
	bool CargaParametros(const wchar_t* billb, float anchura, float altura, int n)
	{
		HRESULT d3dResult;
		...
		//que VS_Main es el nombre del vertex shader en el shader, vsBuffer contendra
		//al puntero del mismo
		bool compileResult = CompileD3DShader(L"BillboardUI.hlsl", "VS_Main", "vs_4_0", &vsBuffer);
		//en caso de no poder cargarse ahi muere la cosa
		if (compileResult == false)
		{
			return false;
		...
		// de los vertices pasamos al pixel shader, note que el nombre del shader es el mismo
		//ahora buscamos al pixel shader llamado PS_Main
		compileResult = CompileD3DShader(L"BillboardUI.hlsl", "PS_Main", "ps_4_0", &psBuffer);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXVECTOR3 poscam,float posx, float posy,float posz, GestorDeLuz* gestor, int n)
	{
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXVECTOR3 poscam,D3DXVECTOR3 vectorFront, float posy,  GestorDeLuz* gestor)
	{
		...
		//mueve la camara
		float difz = -vectorFront.z;
		float difx = -vectorFront.x;
		...
		D3DXMATRIX translationMat;
		D3DXMatrixTranslation(&translationMat, poscam.x + vectorFront.x * 40.f, poscam.y+vectorFront.y*40.f+10.f, poscam.z + vectorFront.z * 40.f);
